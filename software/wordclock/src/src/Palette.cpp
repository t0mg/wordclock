...
namespace Palette
{
  ...
  std::vector<RgbColor> stringToRgb(String payload)
  {
    ...
    for (int i = 0; i < min(static_cast<int>(str.length()), 220); i += 2)
    {
      // Combine two characters into a single byte value
      unsigned int colorIndex = std::stoul(str.substr(i,2), nullptr, 16);

      // Check if the color index is within valid range
      if (colorIndex >= 256)
      {
        // Handle invalid color codes (throw exception, return default value, etc.)
        // For example:
        colorIndex = 0;
      }
